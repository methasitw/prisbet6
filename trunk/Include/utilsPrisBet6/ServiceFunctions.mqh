...
#include <utilsPrisBet6\GetIndicatorBuffers.mqh>
 ...
 double CExpertAdvisor::BasePrice(long dir)
  {
     m_smbinf.Refresh(); // refresh symbol info
     m_smbinf.RefreshRates();
   if(dir==(long)ORDER_TYPE_BUY) return(NormalizeDouble(m_smbinf.Ask(),5));
   if(dir==(long)ORDER_TYPE_SELL) return(NormalizeDouble(m_smbinf.Bid(),5));
   return(WRONG_VALUE);
  }
//+------------------------------------------------------------------+

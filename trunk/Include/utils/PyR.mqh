...
#include <utils\RiskManagement.mqh>
...
//Abandon faith in yourself. Have faith only in the system. 
   ...
   void              PyR();
	...
	virtual long     	   CheckFilterPyr(long type);  
   virtual long         CheckDistance(long type, bool bEntry);
   virtual double LastDealOpenPrice();
   virtual double getTPByRisk(long dir, double lot);
  };
...
//------------------------------------------------------------------ 
double PyR::getTPByRisk(long dir, double lot)
{
   if(!GetBandsBuffers(Bands_handle_PYR,0,90,Base,Upper,Lower,true)) return (-1);
...
// Check Filter
//------------------------------------------------------------------ 
long PyR::CheckFilterPyr(long type)
{   
   if(!CopyBufferAsSeries(EMAFastMaHandle_PYR,0,0,20,true,FastEma)) return(false);
   if(!CopyBufferAsSeries(EMASlowMaHandle_PYR,0,0,20,true,SlowEma)) return(false);

  if(type == ORDER_TYPE_BUY && FastEma[0] > SlowEma[0])  
   {  

...
//+------------------------------------------------------------------+
int TradeSignal_06(bool bEntry)
  {
   int sig=0;

   if(h_rsi==INVALID_HANDLE)
     {
      h_rsi=iRSI(Symbol(),PERIOD_M6,RSIPeriod,RSIprice);
      return(0);
     }
   else
     {
      ...
      printf("rsi_buffer[2]" + rsi_buffer[2] + "rsi_buffer[1]" +rsi_buffer[1] );
//--- perform checking of the condition and set the value for sig
...
//+------------------------------------------------------------------+
int TradeSignal_09(bool bEntry)
  {
   int sig=0;

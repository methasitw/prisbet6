...
input double deviation     = 2.6;          // Standard deviation  // 2.33
...
input double deviationPyr= 2.2;           // Standard deviation  PYR // 2.33


//---indicator parameters pyr 
input int bands_periodThirdPyr= 60;            // Bollinger Bands period PYR
input double deviationThirdPyr= 2.3;           // Standard deviation  PYR // 2.33
// risk management
...
bool Pris::Init(string smb,ENUM_TIMEFRAMES tf)
  {
	...
	Bands_handle_PYR=iBands(_Symbol,periodBB,bands_periodPyr,0,deviationPyr,PRICE_CLOSE);
	Bands_handle_thirdPYR=iBands(_Symbol,periodBB,bands_periodThirdPyr,0,deviationThirdPyr,PRICE_CLOSE);
  ...
  void Pris::OpenPosition(long dir)
  {
             ...
             if(dir!=CheckSignal(dir, true)) return;// if there is no signal for current direction
                ...
                printf(__FUNCTION__+ " ### EMA cruzada   ### "  );      
                  Deal(dir, 0,false);
                  if (dir==ORDER_TYPE_SELL)  IAS =false;
                  else  IAB=false; 
   ...
   if (pyramiding == true)           //piramida
   {
        ...
        double riesgo = Risk*2;
             ...
             if ( (lot +PositionGetDouble(POSITION_VOLUME))  > 15.0 )
               lot = 14.9 -  PositionGetDouble(POSITION_VOLUME);

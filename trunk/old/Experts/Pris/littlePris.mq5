...
input double deviation     = 2.4;          // Standard deviation  // 2.33
...
void Pris::Deal(long dir, int order,bool pyramiding)     // eliminado ratio, necesidad de dos deal diferentes para pir y para entrada inciial ¿?
{
   if (pyramiding == true)           //piramida
   {
            double pips= rm.getPips();
             ...
             printf(__FUNCTION__+ " Posicionamos con un stop de  " + pips/10  + " pips. Volumen de deal " +lot+ " totalVolumen : "+totalVolumen+" Risk : "  + (rm.m_account.FreeMargin()*0.012 + PositionGetDouble(POSITION_PROFIT)));
   ...
   {
          double Risk = rm.m_account.FreeMargin()*0.035;
            double pips= rm.getPips();
            pips = pips * 10 * 0.99; // ponemos el stop de 1/2 del ATR

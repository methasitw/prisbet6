...
input int    PeakVolumen  = 4.5;                   // Volumen considerado suficientemente grande como para cerrar una entrada si toca la dispersion contraria a su movimiento
input int   MaxNumberOrders = 3 ;                // numero de ordenes maxima que pertenece a una posicion
input int    Risk    = 120;
input int    RiskP    = 750;

input double marginIN = 0.05;
...
// Open Position
//------------------------------------------------------------------
  void Pris::PyrPosition(long dir)
  { 
...
void Pris::Deal(long dir, int order,bool pyramiding)     // eliminado ratio, necesidad de dos deal diferentes para pir y para entrada inciial ¿?
{
   ...
   else
   {
            ...
            double pips= rm.getPips();
            pips = pips * 10 * 0.125; // ponemos el stop de 1/8 del ATR

...
#include <utilsPris12\ServiceFunctions.mqh>

input int ATRPeriod =14;
input int MaxMinPeriod = 54;
...
void RiskManagement::RiskManagement()
  ...
  if(!ea.Init(0,smb,tf)) return(false);  // initialize object CExpertAdvisor
   ...
   ExtATRHandleWeek = iATR(NULL,PERIOD_W1,MaxMinPeriod);
...
double RiskManagement::getStopByRisk(long dir, double lot)
{
   ...
   m_symbol.Refresh(); m_symbol.RefreshRates();   
   double risky = m_account.FreeMargin()*0.05;
   if (risky> 2500)  risky = 2500;                         
    double Margin = (risky+PositionGetDouble(POSITION_PROFIT));
  //  double Margin = (PositionGetDouble(POSITION_PROFIT)+500);
    double totalVolumen   =  (PositionGetDouble(POSITION_VOLUME) + lot ) ;       
	double Puntos = Margin /(totalVolumen *10);   //10 dolares x punto
	printf(__FUNCTION__+ " Beneficio de possicion: "+PositionGetDouble(POSITION_PROFIT)+" Margin: " +Margin+ " Volumen " + totalVolumen + " puntos " + Puntos);
  ...
  //------------------------------------------------------------------ 
//------------------------------------------------------------------   
long RiskManagement::MaxMinAbsolut(long dir)
{
    double High[],Low[];
    if(!PositionSelect(m_smb)) return -2;  
      m_symbol.Refresh(); m_symbol.RefreshRates();   
   ...
   if ( OpenPrice <= (down + down * 0.03) && dir ==ORDER_TYPE_BUY )
    {
    // Print(__FUNCTION__ +" El precio "+ ea.BasePrice(ORDER_TYPE_BUY)+" se situa fuera del margen [ " +down+ " : " +up+ " ] [ " +down1+ " : " +up1+ " ] ORDER_TYPE_BUY " );
      return( ORDER_TYPE_BUY);// condition for buy
    }
    else if (OpenPrice >= (up - up*0.03) &&  dir ==ORDER_TYPE_SELL)
    {
     //Print(__FUNCTION__+" El precio "+ ea.BasePrice(ORDER_TYPE_BUY)+" se situa fuera del margen [ " +down+ " : " +up+ " ] [ " +down1+ " : " +up1+ " ] ORDER_TYPE_SELL " );
      return(ORDER_TYPE_SELL);// condition for buy
    }
    return(WRONG_VALUE);
...
{							             // if there is no positions or error, then exit
   ...
   double sl =  ea.BasePrice(dir) * (prct);      
	printf(__FUNCTION__+ " El stop correspondiente a el "+prct+"/100 es : " + sl  );
  return sl;
  }
//------------------------------------------------------------------ 
// Calculo del riesgo expuesto en una primera entrada
//------------------------------------------------------------------   
 double RiskManagement::getRysky()
{
   
   double atr = getN();
    double risky = (m_account.FreeMargin()*2.5*atr);

	printf(__FUNCTION__+ "  risky: " +risky+ " getN() " + getN() );
 
   if (risky> 150) risky = 150;
  ...
  //------------------------------------------------------------------ 
// Calculo del riesgo expuesto en un deal
//------------------------------------------------------------------   
 double RiskManagement::getRyskyDeal()
{
   ...
   m_symbol.Refresh(); m_symbol.RefreshRates();  
    ...
    double risky = (m_account.FreeMargin()*3*atr);

	printf(__FUNCTION__+ "  risky: " +risky+ " atr " + atr );
 
   if (risky> 1000) risky = 1000;
  ...
  double RiskManagement::getLotN()
  {
   ...
   double atr = 0.0;
    ...
    double risky = PositionGetDouble(POSITION_PROFIT) + 10000;
 
  lot = ((0.01 *risky) /(atr*10000));//  10000 dolares x punto
  
  printf(__FUNCTION__+" atr " +  atr + "risky" + risky, 0);
   if (lot >5)
      lot = 5;
   return(lot);
  }
